...
def tollyorder():
    ...
    if request.method == "POST":
        ...
        for row in value:
            query = f'''INSERT INTO OrderTolly (Date,OrderNo,Mitem_ID,Qnty,Amt,OrderMethodID,PayMethodID) VALUES ('{str(row[3])}','{str(row[4])}',{int(row[0])},{int(row[1])},{float(row[2])},{int(row[6])},{int(row[5])})'''
            cur.execute(query)
